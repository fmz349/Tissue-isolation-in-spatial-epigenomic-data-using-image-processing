...
object <- readRDS("~/ME11_H3K27ac_50um/seurat_object")
...
saveRDS(ves,file="~/ME11_H3K27ac_50um/new_vesalius/new_vesalius_embedding.Rds")
...
for(i in 1:30){
  p[[i]] <- imagePlot(ves, dims = i, cex = 12)+theme_void()+coord_flip()+scale_x_reverse()
}
...
for(i in 1:30){
  p3[[i]] <- imagePlot(ves_eh, dims = i, cex = 12)+theme_void()+coord_flip()+scale_x_reverse()
}
...
territoryPlot(seg,cex.pt = 2)+coord_flip()+scale_x_reverse()+theme(
...
# territory isolation
ter <- isolateTerritories(seg,trial = "last")
...
saveRDS(ter,"~/ME11_H3K27ac_50um/new_vesalius/ter")
